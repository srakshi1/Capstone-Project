    ...
    sidebarLayout(
      ...
      mainPanel(
        tabsetPanel(
          ...
          h1('Capstone Project: Environmental Tracker'),
          h5('Page 1 has map of selected country as well as a graph of the log of the feature vs the year for all environmental features available. We fit a linear regression to this graph to determine the log of the future prediction.'),
          h5('Page 2 has graphs of the log of the feature vs the year for the CO2 emission features specifically. The user can also select two different countries.')
        ),
        textOutput("test")
      )
    )
           ...
           sidebarLayout(
             ...
             mainPanel(
               tabsetPanel(
                 tabPanel("Plot",
                          fluidRow(
                            h3('Log of Feature Data and Log of Future Prediction'),
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    p <- p+labs(y = 'Log of feature data')+labs(title = paste(fitted$feature,'and log of prediction in year',fitted$predict_year))
    p
  })
