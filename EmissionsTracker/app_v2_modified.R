...
linreg = function(country,feature,predict_year){
  ...
  y_log_feature_actual <- y_log_feature[is.finite(y_log_feature)]
  year <- year[is.finite(y_log_feature)]
  fit <-lm(formula = y_log_feature_actual ~ year)
  ...
  df_smoothed <- data.frame(year = year, original = y_log_feature_actual, smoothed = linear_smoothed)
